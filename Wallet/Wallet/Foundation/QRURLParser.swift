...
struct QRURLParser {
    static func from(string: String) -> ParserResult? {
        ...
        if parts.count == 1, let address = parts.first, CryptoAddressValidator.isValidTronAddress(address) {
            return ParserResult(
                protocolName: "",
                address: address,
                params: [:]
        ...
        if parts.count == 2, let address = QRURLParser.getAddress(from: parts.last), CryptoAddressValidator.isValidTronAddress(address) {
