...
protocol NetworkProtocol: TrustNetworkProtocol {
    func tokenBalance(for contract: TrustCore.Address, completion: @escaping (_ result: Balance?) -> Void)
    ...
    func tokensList(for address: TrustCore.Address) -> Promise<[TokenObject]>
    func transactions(for address: TrustCore.Address, startBlock: Int, page: Int, contract: String?, completion: @escaping (_ result: ([Transaction]?, Bool)) -> Void)
...
class TrustNetwork: NetworkProtocol {
    ...
    func tokensList(for address: TrustCore.Address, completion: @escaping (([TokenObject]?)) -> Void) {
        provider.request(.getTokens(address: address.description, showBalance: false)) { result in
            switch result {
            case .success(let response):
                do {
    ...
    func transactions(for address: TrustCore.Address, startBlock: Int, page: Int, contract: String?, completion: @escaping (([Transaction]?, Bool)) -> Void) {
        ...
        return Promise { seal in
            provider.request(.search(token: token)) { result in
                ...
                case .success(let response):
                    do {
                        let tokens = try response.map([TokenObject].self)
                        guard let token = tokens.first(where: { $0.address.eip55String == TrustCore.Address(string: token)?.eip55String }) else {
