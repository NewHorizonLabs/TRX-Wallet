...
class Transaction: Object, Decodable {
    ...
    convenience required init(from decoder: Decoder) throws {
        ...
        guard
            let fromAddress = TrustCore.Address(string: from) else {
                let context = DecodingError.Context(codingPath: [TransactionCodingKeys.from],
                                                    debugDescription: "TrustCore.Address can't be decoded as a TrustKeystore.TrustCore.Address")
    ...
    var toAddress: TrustCore.Address? {
        return TrustCore.Address(string: to)
    }

    var fromAddress: TrustCore.Address? {
        return TrustCore.Address(string: from)
    }

    var contractAddress: TrustCore.Address {
        guard
            let operation = operation,
            let contract = operation.contract,
            let contractAddress = TrustCore.Address(string: contract) else {
